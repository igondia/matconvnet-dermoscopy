...
for epoch=start+1:opts.numEpochs
  ...
  params_val=params;
  if(length(params.val)<opts.batchSize)
    params_val.numSubBatches=ceil(params_val.numSubBatches*length(params.val)/opts.batchSize);
  end
  if numel(opts.gpus) <= 1
    [net, state] = processEpoch(net, state, params, 'train') ;
    [net, state] = processEpoch(net,  state, params_val, 'val') ;
    if ~evaluateMode
  ...
  if opts.plotStatistics
    ...
    plots = setdiff(...
      ...
      fieldnames(stats.val)'), {'num', 'time','wseg_bals'},'stable') ;
    for p = plots
      p = char(p) ;
