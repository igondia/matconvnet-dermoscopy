function cnn_testMelanomas_diagnosis_mod_augnocrop(numArch)
...
if(exist([opts.expDir '/net-init.mat'],'file'))
    ...
    %%Only the options
    [~, ~, opts]=netStructure(opts);
else
    [~, ~,opts]=netStructure(opts);
end

opts.imdbPath=regexprep(opts.imdbPath,'.mat','_test_nocrop.mat');
...
opts.train.batchSize = 6*imdb.images.numVariations;
...
% --------------------------------------------------------------------
imDBFolder='./data/db_images_test_nocrop';%imDBPath(1:end-6);
mkdir(imDBFolder);
...
if(numDoneTotImages<numTotalImages)
    ...
    parfor f=1:length(idxTest)
		...
		im=imresize(im,[NaN 2048]);
            ...
            vmin=min(mask(:));
            mask=mask>vmin;
            mask=imdilate(mask,strel('disk',100));
