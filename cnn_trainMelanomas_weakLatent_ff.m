function cnn_trainMelanomas_weakLatent(numArch)
...
opts.train.numArch=numArch;
opts.train.GrayScale=0;
...
[net, info] = cnn_train_weakLatent(net, imdb, @getBatch, opts.train, 'train', find(imdb.images.set == 1),'val',find(imdb.images.set == 2)) ;

% --------------------------------------------------------------------
function [im, pcoords, labels, instanceWeights] = getBatch(imdb, batch)
% --------------------------------------------------------------------
