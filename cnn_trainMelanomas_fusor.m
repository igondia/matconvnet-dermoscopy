function cnn_trainMelanomas_fusor(numArch)
...
load(imdb.images.paths{batch(1)},'pcoord');
...
[H W C]=size(pcoord);
pcoords=zeros(H,W,2,numIm,'single');
im(:,:,:,1)=gather(data);
pcoords(:,:,:,1)=pcoord;
lFolder=length(folder);
for i=2:numIm
%     aux=load(imdb.images.paths{batch(i)},'pcoord');
%     pcoords(:,:,:,i)=aux.pcoord;
    fname=imdb.images.paths{batch(i)}(lFolder+2:end);
    sFile=[sfolder '/' fname];
    aux=load(sFile,'data','pcoord');
    im(:,:,:,i)=gather(aux.data);
    pcoords(:,:,:,i)=aux.pcoord;
end

switch networkType
    case 'simplenn'
      varargout = {im, pcoords,labels};
    case 'dagnn'
      varargout{1} = {'input', im, 'label', labels,'pcoords',pcoords};
end
