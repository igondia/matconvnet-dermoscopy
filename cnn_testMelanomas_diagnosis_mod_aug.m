function cnn_trainMelanomas_diagnosis_mod(numArch,numModel)
...
%%%%%%%%%%%%%%%%%%%READ THE DATASET%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%model_function = str2func(['@model' num2str(numModel)]);
...
if(exist([opts.expDir '/net-init.mat'],'file'))
    ...
    %%Only the options
    [~, net_seg, opts]=model(opts,numModel);
else
    [net, net_seg,opts]=model(opts,numModel);
end


opts.imdbPath=regexprep(opts.imdbPath,'.mat','_test.mat');
if(~exist(opts.imdbPath,'file'))
   
    idxDir=[dataDir 'idx/'];
    load([idxDir '' opts.dbName '.mat']);
    
    imdb = getMelanomasImdb(ch_cases,opts.imSize,opts.imdbPath,opts.trainDB,opts.testDB,opts.numSet,opts.numSets);
    save(opts.imdbPath,'imdb','-v7.3');
end

load(opts.imdbPath,'imdb');
opts.train.batchSize = imdb.images.numVariations;
opts.train.numSubBatches= 1;

if(isfield(net.meta,'vDataMean'))
    imdb.images.vdata_mean=reshape(net.meta.vDataMean,[1 1 3]);
end
...
switch opts.networkType
  ...
  case 'dagnn', trainFn = @cnn_train_dag_mod ;
end
trainidx=[];
...
[net, info] = trainFn(net, net_seg,imdb, @getBatch, opts.train, 'train', trainidx,'val',validx) ;
    ...
    system(['mv ' opts.expDir '/outs-epoch-1.mat ' opts.expDir '/outs-' num2str(numModel) '.mat' ])
end

% --------------------------------------------------------------------
function varargout = getBatch(imdb, batch, networkType,aug,opts)
% --------------------------------------------------------------------
...
%     instanceWeights=gpuArray(instanceWeights);
% end
switch networkType
    case 'simplenn'
    ...
    case 'dagnn'
        if(aug)
            varargout{1} = {'input', im, 'label', labels,'pcoords',pcoords,'imIdx',imIds};%,'instanceWeights',instanceWeights} ;
        else
...
% --------------------------------------------------------------------
function imdb = getMelanomasImdb(idxStruct,imSize,imDBPath,trainDB,testDB,numSet,numSets)
% --------------------------------------------------------------------
imDBFolder='./data/db_images_test';%imDBPath(1:end-6);
mkdir(imDBFolder);
...
if(numSet>0)
    idxTestIdx=numSet:numSets:length(idxTest);
    idxTest=idxTest(idxTestIdx);
end
...
if(numDoneTotImages<numTotalImages)
    ...
    parfor f=1:length(idxTest)
        idxIm=idxTest(f);
        idxCont=1;
        [result numDoneImages]=unix(['ls -1 ' imDBFolder '/' num2str(idxIm) '/*.mat | wc -l']);
        ...
        if(numDoneImages>=numVariations)
            ...
            if(~exist([imDBFolder '/' num2str(idxIm)],'dir'))
                mkdir([imDBFolder '/' num2str(idxIm)]);
            end
            if(~exist([imDBFolder '_icons/' num2str(idxIm)],'dir'))
                mkdir([imDBFolder '_icons/' num2str(idxIm)]);
            end
            %Reading image
            ...
            if(size(im,1) ~=size(mask,1))
                disp(['Error in ' idxStruct(idxIm).impath]);
                %         keyboard;
            else
                ...
                for o=1:size(imor,4)
                    ...
                    try
                        ...
                        for c=1:numCrops
                            ...
                            imgPath=[imDBFolder '/' num2str(idxIm) '/' num2str(idxCont) '.mat'];
                            imgIPath=[imDBFolder '_icons/' num2str(idxIm) '/' num2str(idxCont) '.jpg'];
                        ...
                        keyboard;
                        disp(['Error in image ' num2str(idxIm)])
                    end
                end
            end
        end
...
imdb.images.numVariations = numVariations;
