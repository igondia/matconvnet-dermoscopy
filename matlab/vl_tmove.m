...
%   tensors as VLDT_GPU ones, by adding a fourth column to FORMAT:
%
%       format = {'single', [1  1], 'x0', 'cpu' ;
%                 'double', [10 5], 'x1', 'VLDT_GPU' }
%
%   Push and pull operations are required to use arrays that match the
%   specifications exactly, including being a CPU or VLDT_GPU array
...
%
%   InPlace::
%     If specified, updates any VLDT_GPU array in place (CPU arrays are
%     always processed by copying). It is a (slightly) unsafe
%     operation. It must be used with both `push` and `pull`
%     commands. A pushed VLDT_GPU array must not be deleted before is
%     pulled again. If the array is deleted between a `push` and a
%     `pull`, the system may write unallocated VLDT_GPU memory.
