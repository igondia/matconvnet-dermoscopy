function [res, net]= vl_simplenn_mask(net, x, pcoords, dzdy, res, varargin)
...
%
%   Networks can run either on the CPU or VLDT_GPU. Use VL_SIMPLENN_MOVE()
...
for i=1:n
  ...
  switch l.type
    ...
    case 'circpool'
        res(i+1).x = vl_nncircpool(res(i).x, l.pool, 'pad', l.pad, 'stride', l.stride,'method', l.method,l.opts{:},cudnn{:}) ;
        
    case 'circpoolmask'
        pcoordsr=pcoords;
        pcoordsr(pcoordsr<0)=-pcoordsr(pcoordsr<0);
        pcoordsr = imresize(pcoordsr,[size(res(i).x,1) size(res(i).x,2)]);%,'Method','nearest'
        %pcoordsr = imresize(pcoordsr,[size(res(i).x,1) size(res(i).x,2)],'Method','nearest');
        pcoordsr(:,:,1,:)=bsxfun(@rdivide,pcoordsr(:,:,1,:),max(max(pcoordsr(:,:,1,:),[],1),[],2));
%         sizey=floor(size(res(i).x,1)/2);
%         sizex=floor(size(res(i).x,2)/2);
%         [X Y]=meshgrid(-sizex:sizex,sizey:-1:-sizey);
% %         pcoordsr(:,:,1,:)=repmat(sqrt(X.^2+Y.^2),[1 1 1 size(res(i).x,4)]);
%         a=atan2(Y,X);a(a<0)=2*pi+a(a<0);
%         pcoordsr(:,:,2,:)=repmat(a,[1 1 1 size(res(i).x,4)]);
%         pcoordsr(:,:,1,:)=bsxfun(@rdivide,pcoordsr(:,:,1,:),max(max(pcoordsr(:,:,1,:),[],1),[],2));
        %OJO!! Necesito normalizar los valores de radius a 1
%         j=1;
%         numR=l.pool(1);
%         numA=l.pool(2);
%         R=pcoordsr(:,:,1,j);
...
%         %Radius Uniform
% %         stepR=maxR/numR;
%         %radius=floor(pcoordsr(:,:,1,j)/stepR)+1;
%         %Non-uniform
%         stepR=maxR.^2/numR;
%         radius=floor(sign(pcoordsr(:,:,1,j)).*pcoordsr(:,:,1,j).^2/stepR)+1;
%         radius=min(radius,numR);
%         angles=floor(pcoordsr(:,:,2,j)/stepA)+1;
%         angles=min(angles,numA);
%         idx=(radius-1)*numA+angles;
%         idx
%         h=hist(idx(:),unique(idx(:)));[unique(idx(:))';h]
%         encontrados=sum(ismember(1:numR*numA,unique(idx)));
%         if(encontrados<numR*numA)
%             fprintf('Error\n');
%             keyboard;
%         end
%         pause;
        ...
        res(i+1).x = vl_nncircpool_mask(res(i).x, pcoordsr, l.pool, 'pad', l.pad, 'overlap', l.overlap, 'method', l.method, l.opts{:}, cudnn{:}) ;       
        if(gpuMode>0)
            clear pcoordsr;
        end
    case 'aggregation'
        validos=single(pcoords(:,:,1,:)>0);
        validos = imresize(validos,[size(res(i).x,1) size(res(i).x,2)],'Method','nearest');
%         validos=ones([size(res(i).x,1) size(res(i).x,2) 1 size(res(i).x,4)],'single');
        if(gpuMode>0)
            validos = gpuArray(validos) ;
        end
        res(i+1).x = vl_nnaggregation(res(i).x, validos, 'r', l.r,'method', l.method, l.opts{:}, cudnn{:}) ;       
        if(gpuMode>0)
            clear validos;
        end
    
    case 'weakloss'
      pcoordsr = imresize(pcoords,[size(res(i).x,1) size(res(i).x,2)],'Method','nearest'); 
      l.lambda=zeros(size(l.lambda),'single');      
      if(gpuMode>0)
        pcoordsr = gpuArray(pcoordsr) ;
        l.lambda = gpuArray(l.lambda) ;
      end
      %Para limitar las exponenciales internas
      res(i).x=min(res(i).x,50);
      [res(i+1).x, new_lambda]= vl_nnweakloss(res(i).x.*l.gamma, pcoordsr, l.labels, l.lambda,l.A,l.b,l.beta,'maxLambda',l.maxLambda) ;
%       keyboard;
%         hist(gather(res(i).x(:)),100)
      net.layers{i}.lambda=new_lambda;
% 
%         predictions=squeeze(gather(res(i).x));
...
%       idx=1;
%       P=exp(predictions(idx,:)+((l.labels(:,idx)'>0)-(l.labels(:,idx)'==0)).*new_lambda(1:2:end,idx)');
%       e=exp(predictions(idx,:));
%       [(e/sum(e))' l.labels(:,idx) new_lambda(1:2:end,idx)]
%       [e/sum(e);P/sum(P)]
%       [e/sum(e)-P/sum(P)]*100
         if(sum(isinf(res(i+1).x(:))) || sum(isnan(res(i+1).x(:))))
            error('Error Nan');
          end
      if(gpuMode>0)
          clear pcoordsr;
    ...
    case 'simpool'
      res(i+1).x = vl_nnsimpool(res(i).x, cudnn{:}) ;
      
    case {'normalize', 'lrn'}
      res(i+1).x = vl_nnnormalize(res(i).x, l.param) ;

    case 'softmax'
      res(i+1).x = vl_nnsoftmax(res(i).x,[],'gamma',l.gamma) ;
    ...
    case 'simplenorm'
      res(i+1).x = (res(i).x-l.mean)/l.std;
%       [min(res(i+1).x(:)) max(res(i+1).x(:))]
    case 'dropout'
      if testMode
        res(i+1).x = res(i).x ;
      else
...
%       end
    ...
    case 'fuseInputs'  
      res(i+1).x = vl_nnfuseInputs(res(i).x,l.wmod);
        
    case 'modulateInputs'  
      res(i+1).x = vl_nnmodulateInputs(res(i).x,l.wmod);
      
    otherwise
      error('Unknown layer type ''%s''.', l.type) ;
  end
%   [num2str(1) ' ' num2str(i) ' ' net.layers{i}.type ' ' num2str(size(res(i+1).x))]
%   keyboard;
    ...
    switch l.type
      ...
      case 'pool'
        res(i).dzdx = vl_nnpool(res(i).x, l.pool, res(i+1).dzdx, ...
                                ...
                                cudnn{:}) ;
       case 'circpool'
            auxdzdx = vl_nncircpool(res(i).x, l.pool, res(i+1).dzdx, ...
                    'pad', l.pad, 'stride', l.stride, 'method', l.method) ;
                res(i).dzdx=auxdzdx;
                
      case 'circpoolmask'
            %Fuera de la lesión a positivo
            pcoordsr=pcoords;
            pcoordsr(pcoordsr<0)=-pcoordsr(pcoordsr<0);
            pcoordsr = imresize(pcoordsr,[size(res(i).x,1) size(res(i).x,2)]);
            %pcoordsr = imresize(pcoordsr,[size(res(i).x,1) size(res(i).x,2)],'Method','nearest');
            pcoordsr(:,:,1,:)=bsxfun(@rdivide,pcoordsr(:,:,1,:),max(max(pcoordsr(:,:,1,:),[],1),[],2));
%           %Fuera de la lesión también a 1.0
%           pcoordsr(pcoordsr<0)=1.0;
%           pcoordsr(:,:,1,:)=bsxfun(@rdivide,pcoordsr(:,:,1,:),max(max(pcoordsr(:,:,1,:),[],1),[],2));
%           sizey=floor(size(res(i).x,1)/2);
%           sizex=floor(size(res(i).x,2)/2);
%           [X Y]=meshgrid(-sizex:sizex,sizey:-1:-sizey);
...
%         pcoordsr(:,:,1,:)=bsxfun(@rdivide,pcoordsr(:,:,1,:),max(max(pcoordsr(:,:,1,:),[],1),[],2));
            ...
            res(i).dzdx = vl_nncircpool_mask(res(i).x, pcoordsr, l.pool, res(i+1).dzdx, ...
                    'pad', l.pad, 'overlap', l.overlap, 'method', l.method) ;
      	    if(gpuMode>0)
            	clear pcoordsr;
            end	
      case 'aggregation'
...
%         validos=ones([size(res(i).x,1) size(res(i).x,2) 1 size(res(i).x,4)],'single');
        ...
        res(i).dzdx = vl_nnaggregation(res(i).x, validos, res(i+1).dzdx, 'r', l.r,'method', l.method, l.opts{:}, cudnn{:}) ;       
        if(gpuMode>0)
            clear validos;
        end
      case 'weakloss'  
          ...
          [res(i).dzdx, new_lambda]= vl_nnweakloss(res(i).x.*l.gamma, pcoordsr, l.labels, l.lambda, l.A,l.b,l.beta,res(i+1).dzdx,'maxLambda',l.maxLambda);         

          % 
%       predictions=squeeze(gather(res(i).x));
%       predictions=vl_nnsoftmax(l.gamma*predictions);
%       validos=gather(pcoordsr(:,:,1,:)>=0);
%       predictions=predictions.*repmat(validos,[1 1 size(predictions,3) 1]);
%       predictions=squeeze(sum(sum(predictions,1),2))';    
%       predictions=bsxfun(@rdivide,predictions,squeeze(sum(sum(validos,2),1)));
%       idx=1;
%       
%       [predictions(idx,:)' l.labels(:,idx) new_lambda(1:2:end,idx) squeeze(sum(sum(res(i).dzdx(:,:,:,idx),1),2))*100]
      
      

          if(sum(isinf(res(i).dzdx(:))) || sum(isnan(res(i).dzdx(:))))
%             keyboard;
            error('Error Nan');
          end
      ...
      case 'simpool'
         res(i).dzdx = vl_nnsimpool(res(i).x, res(i+1).dzdx, ...                                   
                                    cudnn{:}) ;
      ...
      case 'softmax'
        res(i).dzdx = vl_nnsoftmax(res(i).x, res(i+1).dzdx,l.gamma) ;
      ...
      case 'fuseInputs'  
        res(i).dzdx = vl_nnfuseInputs(res(i).x,l.wmod,res(i+1).dzdx);
      
      case 'modulateInputs'  
        res(i).dzdx = vl_nnmodulateInputs(res(i).x,l.wmod,res(i+1).dzdx);
...
%     [-1 i size(res(i).x) size(res(i+1).dzdx)]
     switch l.type
