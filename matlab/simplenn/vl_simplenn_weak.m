function [res net]= vl_simplenn_weak(net, x, pcoords, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'circpoolmask'
        ...
        res(i+1).x = vl_nncircpool_mask(res(i).x, pcoordsr, l.pool, 'pad', l.pad, 'stride', l.stride, 'method', l.method, l.opts{:}, cudnn{:}) ;       
        if(gpuMode>0)
            clear pcoordsr;
        end
    case 'aggregation'
...
%   [num2str(1) ' ' num2str(i) ' ' net.layers{i}.type ' ' num2str(size(res(i+1).x))]
  ...
  if(i<n && isempty(segoutputs))
      if(strcmp(net.layers{i+1}.type,'aggregation') || strcmp(net.layers{i+1}.type,'loss'))
          segoutputs=res(i).x;
      end
  end
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'circpoolmask'
            ...
            auxdzdx = vl_nncircpool_mask(res(i).x, pcoordsr, l.pool, res(i+1).dzdx, ...
                    'pad', l.pad, 'stride', l.stride, 'method', l.method) ;
            res(i).dzdx=auxdzdx;
      	    if(gpuMode>0)
            	clear pcoordsr;
