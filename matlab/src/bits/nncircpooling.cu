...
#include "nncircpooling.hpp"
#include "impl/circpooling.hpp"
...
switch (method) { \
case vlCircPoolingAverage : DISPATCH2(deviceType, circpooling_average) ; break ; \
case vlCircPoolingMax : DISPATCH2(deviceType, circpooling_max) ; break ; \
default: assert(false) ; return VLE_Unknown ; \
}
...
vl::ErrorCode
vl::nncircpooling_forward(vl::Context& context,
                      vl::Tensor output,
                      vl::Tensor data,
...
/*      }
  ...
  return context.passError(status, "nncircpooling_forward") ;
}

/* ---------------------------------------------------------------- */
/*                                               nncircpooling_backward */
/* ---------------------------------------------------------------- */
...
#define DISPATCH_circpooling_average(deviceType, type) \
status = vl::impl::circpooling_average<deviceType, type>::backward \
...
#define DISPATCH_circpooling_max(deviceType, type) \
status = vl::impl::circpooling_max<deviceType, type>::backward \
...
vl::ErrorCode
vl::nncircpooling_backward(Context& context,
                       Tensor derData,
                       Tensor data,
                       Tensor derOutput,
                       PoolingMethod method,
...
/*#if ENABLE_GPU
  ...
  return context.passError(status, "nncircpooling_backward") ;
}
