...
{
#if ENABLE_GPU
  // so that ~Context does not crash if MATLAB resets the VLDT_GPU in the mean time
  validateGpu() ;
#endif
}
...
// Do noting if the VLDT_GPU is not initialized, otherwise invalidate it
// if needed
vl::ErrorCode
MexContext::validateGpu()
{
  ...
  if (!gpuIsInitialized) {
#ifndef NDEBUG
    std::cout<<"MexContext:: VLDT_GPU reset detected; invalidating the VLDT_GPU state"<<std::endl ;
#endif
...
// Initialize VLDT_GPU; also make sure that it was not reset by MATLAB
vl::ErrorCode
...
/*
 The MexTensor class helps handling MATLAB CPU and VLDT_GPU arrays.
 ...
 CPU/VLDT_GPU memory can occur. In this case, MATLAB throws an error
                          ...
                          TensorShape const & newShape, bool fillWithZeros)
{
  ...
  // allocate the memory on CPU or VLDT_GPU
  void * newMemory = NULL ;
...
vl::MexTensor::init(mxArray const * array_)
{
...
#endif
  {
    if (!mxIsNumeric(newArray)) {
      mexErrMsgTxt("An input is not a numeric array (or VLDT_GPU support not compiled).") ;
    }
...
void vl::print(char const * str, vl::MexTensor const & tensor)
{
  ...
  switch (tensor.getDeviceType()) {
    ...
    case vl::VLDT_GPU : dev = "VLDT_GPU" ; break ;
  }
