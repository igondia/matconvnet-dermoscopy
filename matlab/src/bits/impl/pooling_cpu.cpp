...
struct acc_min
{
  inline acc_min(int poolHeight, int poolWidth, type derOutput = 0)
  :
  value(std::numeric_limits<type>::infinity()),
  derOutput(derOutput),
  derDataActivePt(NULL)
...
template <typename type>
  struct pooling_min<vl::VLDT_CPU, type>
  {
    static vl::ErrorCode
    forward(type* pooled,
            type const* data,
            ...
            size_t padTop, size_t padBottom, size_t padLeft, size_t padRight)
    {
      pooling_forward_cpu<type, acc_min<type> > (pooled,
                                                 data,
                                                 height, width, depth,
                                                 poolHeight, poolWidth,
                                                 strideY, strideX,
    ...
    backward(type* derData,
             ...
             size_t padLeft, size_t padRight)
    {
      pooling_backward_cpu<type, acc_min<type> > (derData,
                                                  data, derOutput,
                                                  height, width, depth,
                                                  poolHeight, poolWidth,
                                                  strideY, strideX,
