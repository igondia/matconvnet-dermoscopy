 ...
 const int padTop)
{
  ...
  if (pooledIndex < pooledVolume) {
    ...
    for (int y = y1 ; y < y2 ; ++y) {
      for (int x = x1 ; x < x2 ; ++x) {
        bestValue = min(bestValue, data[y * width + x]) ;
      }
    }
    pooled[pooledIndex] = bestValue ;
  }
...
template <typename type>
  struct pooling_min<vl::VLDT_GPU, type>
  {
    static vl::ErrorCode
    forward(type* pooled,
            type const* data,
             ...
             size_t padLeft, size_t padRight)
    {
      ...
      pooling_min_backward_kernel<type>
      <<< divideAndRoundUp(pooledVolume, VL_CUDA_NUM_THREADS), VL_CUDA_NUM_THREADS >>>
      (derData, data, derOutput,
       pooledHeight, pooledWidth, pooledVolume,
       height, width,
