                 ...
                 int nin, mxArray const *in[])
{
  ...
  while ((opt = vlmxNextOption (in, nin, options, &next, &optarg)) >= 0) {
    switch (opt) {
      ...
      case opt_method :
        ...
        } else if (vlmxIsEqualToStringI(optarg, "min")) {
                  method = vl::vlPoolingMin ;  
        } else {
  ...
  if (verbosity > 0) {
    mexPrintf("vl_nnpool: %s; %s", backMode?"backward":"forward", (data.getDeviceType()==vl::VLDT_GPU) ? "VLDT_GPU" : "CPU") ;
