...
#include "bits/nncircpooling.hpp"
                 ...
                 int nin, mxArray const *in[])
{
  ...
  int padTop = 0 ;
  int padBottom = 0 ;
  vl::PoolingMethod method = vl::vlCircPoolingMax ;
  bool backMode = false ;
    ...
    switch (opt) {
      ...
      case opt_method :
        ...
        if (vlmxIsEqualToStringI(optarg, "max")) {
          method = vl::vlCircPoolingMax ;
        } else if (vlmxIsEqualToStringI(optarg, "avg")) {
          method = vl::vlCircPoolingAverage ;
        } else {
 ...
 /* if (data.getHeight() + (padTop+padBottom) < poolHeight ||
  ...
  if (verbosity > 0) {
    mexPrintf("vl_nncircpool: %s; %s", backMode?"backward":"forward", (data.getDeviceType()==vl::VLDT_GPU) ? "VLDT_GPU" : "CPU") ;
    ...
    mexPrintf("vl_nncircpool: stride: [%d %d], pad: [%d %d %d %d]\n",
              strideY, strideX,
              padTop, padBottom, padLeft, padRight) ;
    vl::print("vl_nncircpool: data: ", data) ;
    mexPrintf("vl_nncircpool: pooling: %d x %d\n", poolRings, poolAngles);
    mexPrintf("vl_nncircpool: method: %s\n", (method == vl::vlCircPoolingMax) ? "max" : "avg") ;
    if (backMode) {
      vl::print("vl_nncircpool: derOutput: ", derOutput) ;
      vl::print("vl_nncircpool: derData: ", derData) ;
    } else {
      vl::print("vl_nncircpool: output: ", output) ;
    }
  }
  ...
  vl::ErrorCode error ;
  if (!backMode) {
    error = vl::nncircpooling_forward(context,
                                  output, data,
                                  method,
                                  poolRings,poolAngles,
                                  strideY, strideX,
    ...
    error = vl::nncircpooling_backward(context,
