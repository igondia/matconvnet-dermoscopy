...
void Batch::returnItem(Batch::Item * item)
{
  ...
  if (item->state == Item::fetch &&
      ...
      gpuMode) {
#if ENABLE_GPU
    LOG(2) << "push to VLDT_GPU the pack" ;
    cudaError_t cerror ;
...
void Batch::setGpuMode(bool VLDT_GPU)
{
  tthread::lock_guard<tthread::mutex> lock(mutex) ;
#if ENABLE_GPU
  if (VLDT_GPU) {
...
void ReaderTask::entryPoint()
{
  ...
  while (true) {
    ...
    if (batch->gpuMode && batch->gpuDevice != gpuDevice) {
      LOG(2) << "reader " << index << " setting VLDT_GPU device" ;
...
#else
            snprintf(item->errorMessage, sizeof(item->errorMessage), "VLDT_GPU support not compiled.") ;
            break;
#endif
          } else {
                 ...
                 int nin, mxArray const *in[])
{
  ...
  while ((opt = vlmxNextOption (in, nin, options, &next, &optarg)) >= 0) {
    switch (opt) {
      ...
      case opt_gpu :
#ifndef ENABLE_GPU
        vlmxError(VLMXE_IllegalArgument, "Not compiled with VLDT_GPU support.") ;
#endif
        gpuMode = true ;
        break ;
  ...
  if (!sameAsPrefeteched) {
    ...
    if (verbosity >= 2) {
      mexPrintf("vl_imreadjpeg: VLDT_GPU mode: %s\n", gpuMode?"yes":"no") ;
