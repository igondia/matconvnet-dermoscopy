...
%vl_compilenn('EnableGpu',true,'EnableCudnn',true,'EnableDouble', false,'CudaRoot','/usr/local/cuda','CudnnRoot','/mnt/workspace/cudnn/')
...
% Files that are compiled as CPP or CU depending on whether VLDT_GPU support
% is enabled.
...
lib_src{end+1} = fullfile(root,'matlab','src','bits',['nncircpooling_mask.' ext]) ;
lib_src{end+1} = fullfile(root,'matlab','src','bits',['nnsimpooling.' ext]) ;
lib_src{end+1} = fullfile(root,'matlab','src','bits',['nnaggregation.' ext]) ;
lib_src{end+1} = fullfile(root,'matlab','src','bits',['nnweakloss.' ext]) ;
...
mex_src{end+1} = fullfile(root,'matlab','src',['vl_nncircpool_mask.' ext]) ;
mex_src{end+1} = fullfile(root,'matlab','src',['vl_nnsimpool.' ext]) ;
mex_src{end+1} = fullfile(root,'matlab','src',['vl_nnaggregation.' ext]) ;
mex_src{end+1} = fullfile(root,'matlab','src',['vl_nnweakloss.' ext]) ;
...
lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','circpooling_mask_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','aggregation_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','weakloss_cpu.cpp') ;
lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','simpooling_cpu.cpp') ;
...
if opts.enableGpu
  ...
  lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','circpooling_mask_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','simpooling_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','aggregation_gpu.cu') ;
  lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','weakloss_gpu.cu') ;
...
if opts.enableGpu
  ...
  % Set the CUDA arch string (select VLDT_GPU architecture)
...
% nvcc: compile VLDT_GPU
flags.nvcc = horzcat(flags.cc, ...
                     {opts.cudaArch}, ...
                     {sprintf('-I"%s"', fullfile(matlabroot, 'extern', 'include'))}, ...
                     {sprintf('-I"%s"', fullfile(matlabroot, 'toolbox','distcomp','VLDT_GPU','extern','include'))}, ...
...
for v = {'5.5', '6.0', '6.5', '7.0', '7.5', '8.0', '8.5', '9.0'}
  switch computer('arch')
    ...
    case 'win64'
      paths{end+1} = sprintf('C:\\Program Files\\NVIDIA VLDT_GPU Computing Toolkit\\CUDA\\v%s\\bin\\nvcc.exe', char(v)) ;
  end
end
...
% --------------------------------------------------------------------
opts.verbose && fprintf('%s:\tCUDA: determining VLDT_GPU compute capability (use the ''CudaArch'' option to override)\n', mfilename);
try
  ...
  opts.verbose && fprintf(['%s:\tCUDA: cannot determine the capabilities of the installed VLDT_GPU; ' ...
