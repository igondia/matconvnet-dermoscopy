...
%     particularly important on the VLDT_GPU, where memory is
...
%   v = obj.getVarIndex(obj.layers(l).outputs) ;
% [l size(obj.vars(v).value)]
...
inverse_order=fliplr(obj.executionOrder);
if(opts.levels_update>0)
    inverse_order=inverse_order(1:opts.levels_update);
end
for l = inverse_order
%   obj.layers(l).name  
  time = tic ;
